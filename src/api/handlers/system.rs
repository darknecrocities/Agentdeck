...
pub async fn ensure_screen_stream_running() {
    ...
    if !stream.is_running.load(Ordering::SeqCst) {
        ...
        tokio::spawn(async move {
            #[cfg(target_os = "macos")]
            {
                // Primary: Try hardware ScreenCaptureKit helper (agentdeck-screen-streamer)
                let helper_paths = [
                ...
                use base64::Engine;
                let mut hardware_streamer_success = false;

                if let Some(bin) = helper_bin {
                    tracing::info!("Attempting hardware ScreenCaptureKit streamer: {}", bin);
                            ...
                            *child_handle.lock().await = Some(child);
                            ...
                            while is_running.load(Ordering::Relaxed) {
                                ...
                                if now_ts - last_requested_at.load(Ordering::Relaxed) > 20 {
                                    tracing::info!("Screen stream idle timeout, stopping hardware screen streamer");
                                    break;
                                }
                                ...
                                if let Ok(Ok(_)) = tokio::time::timeout(std::time::Duration::from_millis(500), stdout.read_exact(&mut len_buf)).await {
                                    ...
                                    if frame_len > 0 && frame_len < 5_000_000 {
                                        ...
                                        if let Ok(Ok(_)) = tokio::time::timeout(std::time::Duration::from_millis(500), stdout.read_exact(&mut frame_data)).await {
                                            hardware_streamer_success = true;
                                    ...
                                    // Check if process exited early (e.g. TCC denied)
                                    let mut lock = child_handle.lock().await;
                                    if let Some(ref mut proc) = *lock {
                                        if let Ok(Some(_status)) = proc.try_wait() {
                                            tracing::warn!("agentdeck-screen-streamer exited early (likely TCC prompt pending)");
                                            *lock = None;
                                            break;
                                        }
                                    }
                ...
                // Fallback: If hardware streamer didn't output frames (or exited due to TCC), try screencapture CLI loop
                if !hardware_streamer_success && is_running.load(Ordering::Relaxed) {
                    tracing::info!("Falling back to system screencapture CLI capture loop...");
                    ...
                    while is_running.load(Ordering::Relaxed) {
                        ...
                        let result = tokio::process::Command::new(screencapture_bin)
                            .args(["-x", "-t", "jpg", "-m", frame_path.to_str().unwrap_or("/tmp/agentdeck_ss.jpg")])
                            .output()
                            .await;
