...
impl OllamaAdapter {
    pub fn new(binary: Option<String>) -> Self {
        Self {
            binary: binary.unwrap_or_else(|| "ollama".to_string()),
    ...
    fn resolve_binary(&self) -> String {
        ...
        let candidates = vec!["ollama", "/usr/local/bin/ollama", "/opt/homebrew/bin/ollama"];
        for c in candidates {
...
impl AgentAdapter for OllamaAdapter {
    ...
    async fn start(&self, request: AgentStartRequest, event_bus: EventBus) -> anyhow::Result<AgentSession> {
        ...
        let model = request.model.unwrap_or_else(|| "codellama".to_string());
        ...
        cmd.arg("run").arg(&model).arg(&request.prompt);
    ...
    async fn continue_session(&self, _session_id: &str, _event_bus: EventBus) -> anyhow::Result<()> {
        Ok(())
    }
